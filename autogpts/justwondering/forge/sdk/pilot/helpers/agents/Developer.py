...
from forge.sdk.pilot.helpers.cli import build_directory_tree, run_command_until_success, execute_command_and_check_cli_response, debug
from forge.sdk.pilot.const.function_calls import FILTER_OS_TECHNOLOGIES, DEVELOPMENT_PLAN, EXECUTE_COMMANDS, GET_TEST_TYPE, DEV_TASKS_BREAKDOWN, IMPLEMENT_TASK
from forge.sdk.pilot.database.database import save_progress, get_progress_steps, save_file_description
...
class Developer(Agent):
    ...
    def start_coding(self):
        ...
        if self.project.skip_steps is None:
            self.project.skip_steps = False if (
                'skip_until_dev_step' in self.project.args and self.project.args['skip_until_dev_step'] == '0') else True
    ...
    def implement_task(self, dev_task, prompt):
        convo_dev_task = AgentConvo(self)
        task_description = convo_dev_task.send_message(prompt, {
            "directory_tree": self.project.get_directory_tree(True),
            "development_tasks": self.project.development_plan,
            "files": self.project.get_all_coded_files(),
        })
        task_steps = convo_dev_task.send_message(
            'development/parse_task.prompt', {}, IMPLEMENT_TASK)
        return task_steps

    def implement_task(self, i, development_task=None):
        print(colored(f'Implementing task #{i + 1}: ', 'green', attrs=[
              'bold']) + colored(f' {development_task["description"]}\n', 'green'))
    ...
    def execute_task(self, convo, task_steps, test_command=None, reset_convo=True, test_after_code_changes=True, continue_development=False, development_task=None):
        ...
        for (i, step) in enumerate(task_steps):
            ...
            if step['type'] == 'command':
                ...
                run_command_until_success(
                    data['command'], data['timeout'], convo, additional_message=additional_message)
                ...
                print(
                    f'Implementing code changes for `{step["code_change_description"]}`')
                code_monkey = CodeMonkey(self.project, self)
                updated_convo = code_monkey.implement_code_changes(
                    convo, step['code_change_description'], i)
                ...
                human_intervention_description = step['human_intervention_description'] + colored('\n\nIf you want to run the app, just type "r" and press ENTER and that will run `' + self.run_command + '`', 'yellow', attrs=[
                                                                                                  'bold']) if self.run_command is not None else step['human_intervention_description']
                ...
                if user_feedback is not None and user_feedback != 'continue':
                    debug(convo, user_input=user_feedback,
            ...
            if test_command is not None and ('check_if_fixed' not in step or step['check_if_fixed']):
                ...
                if should_rerun_command == 'NO':
                    ...
                    cli_response, llm_response = execute_command_and_check_cli_response(
                        test_command['command'], test_command['timeout'], convo)
        ...
        self.run_command = convo.send_message(
    ...
    def continue_development(self, iteration_convo, continue_description=''):
        while True:
            user_description = ('Here is a description of what should be working: \n\n' + colored(
                continue_description, 'blue', attrs=['bold']) + '\n') if continue_description != '' else ''
            user_description = 'Can you check if the app works please? ' + user_description + '\nIf you want to run the app, ' + \
                colored('just type "r" and press ENTER and that will run `' +
                        self.run_command + '`', 'yellow', attrs=['bold'])
            continue_description = ''
            ...
            if user_feedback is not None:
                ...
                task_steps = iteration_convo.send_message(
                    'development/parse_task.prompt', {}, IMPLEMENT_TASK)
    ...
    def set_up_environment(self):
        ...
        while user_input.lower() != 'done':
            user_input = styled_text(
                self.project, 'Please set up your local environment so that the technologies above can be utilized. When you\'re done, write "DONE"')
            ...
            if llm_response != 'DONE':
                ...
                if installation_commands is not None:
                    for cmd in installation_commands:
                        run_command_until_success(
                            cmd['command'], cmd['timeout'], self.convo_os_specific_tech)

        logger.info(
            'The entire tech stack neede is installed and ready to be used.')
    ...
    def test_code_changes(self, code_monkey, convo):
        ...
        if test_type == 'command_test':
            run_command_until_success(
                command['command'], command['timeout'], convo)
        elif test_type == 'automated_test':
            code_monkey.implement_code_changes(
                convo, automated_test_description, 0)
        elif test_type == 'manual_test':
            # TODO make the message better
            user_feedback = self.project.ask_for_human_intervention(
                'Message from forge.sdk.pilot: I need your help. Can you please test if this was successful?',
                manual_test_description
            )
            if user_feedback is not None:
                debug(convo, user_input=user_feedback,
                      issue_description=manual_test_description)
