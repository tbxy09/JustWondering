...
from forge.sdk.pilot.database.database import get_command_run_from_hash_id, save_command_run
from forge.sdk.pilot.const.function_calls import DEBUG_STEPS_BREAKDOWN

from forge.sdk.pilot.utils.questionary import styled_text
from forge.sdk.pilot.const.code_execution import MAX_COMMAND_DEBUG_TRIES, MIN_COMMAND_RUN_TIME, MAX_COMMAND_RUN_TIME, MAX_COMMAND_OUTPUT_LENGTH
...
def execute_command(project, command, timeout=None, force=False):
    ...
    if not force:
        print(colored(f'\n--------- EXECUTE COMMAND ----------',
              'yellow', attrs=['bold']))
        print(colored(f'Can i execute the command: `') + colored(command,
              'yellow', attrs=['bold']) + colored(f'` with {timeout}ms timeout?'))
    ...
    if command_run is not None and project.skip_steps:
        ...
        print(colored(
            f'Restoring command run response id {command_run.id}:\n```\n{command_run.cli_response}```', 'yellow'))
    ...
    try:
        while True and return_value is None:
            ...
            if timeout is not None:
                print(colored(f'\rt: {round(elapsed_time * 1000)}ms : ',
                      'white', attrs=['bold']), end='', flush=True)
            ...
            if stderr_line:
                ...
                # Print with different color for distinction
                print(colored('CLI ERROR:', 'red') + stderr_line, end='')
...
def build_directory_tree(path, prefix="", ignore=None, is_last=False, files=None, add_descriptions=False):
    ...
    if os.path.isdir(path):
        ...
        output += prefix + "|-- " + os.path.basename(path) + ((' - ' + files[os.path.basename(
            path)].description + ' ' if files and os.path.basename(path) in files and add_descriptions else '')) + "/\n"

        # List items in the directory
        items = os.listdir(path)
        for index, item in enumerate(items):
            item_path = os.path.join(path, item)
            output += build_directory_tree(item_path, prefix + indent,
                                           ignore, index == len(items) - 1, files, add_descriptions)
    ...
    for i in range(MAX_COMMAND_DEBUG_TRIES):
        ...
        debugging_plan = convo.send_message('dev_ops/debug.prompt',
                                            {'command': command['command'] if command is not None else None,
                                                'user_input': user_input, 'issue_description': issue_description},
