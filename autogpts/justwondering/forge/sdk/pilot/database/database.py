...
import psycopg2
import os
from forge.sdk.pilot.const.common import PROMPT_DATA_TO_IGNORE
from forge.sdk.pilot.logger.logger import logger
from psycopg2.extensions import quote_ident

from forge.sdk.pilot.utils.utils import hash_data
from forge.sdk.pilot.database.models.components.base_models import database
from forge.sdk.pilot.database.models.user import User
...
from forge.sdk.pilot.database.models.project_description import ProjectDescription
from forge.sdk.pilot.database.models.user_stories import UserStories
from forge.sdk.pilot.database.models.user_tasks import UserTasks
from forge.sdk.pilot.database.models.architecture import Architecture
from forge.sdk.pilot.database.models.development_planning import DevelopmentPlanning
from forge.sdk.pilot.database.models.development_steps import DevelopmentSteps
from forge.sdk.pilot.database.models.environment_setup import EnvironmentSetup
from forge.sdk.pilot.database.models.development import Development
from forge.sdk.pilot.database.models.file_snapshot import FileSnapshot
from forge.sdk.pilot.database.models.command_runs import CommandRuns
from forge.sdk.pilot.database.models.user_inputs import UserInputs
...
TABLES = [User, App, ProjectDescription, UserStories, UserTasks, Architecture, DevelopmentPlanning,
          DevelopmentSteps, EnvironmentSetup, Development, FileSnapshot, CommandRuns, UserInputs, File, ProjectM, ProjectFiles]
    ...
    try:
        ...
        if args.get('user_id') is not None:
            try:
                ...
                user = save_user(
                    args['user_id'], args['email'], args['password'])
...
def get_db_model_from_hash_id(model, app_id, previous_step):
    try:
        db_row = model.get((model.app == app_id) & (
            model.previous_step == previous_step))
    except DoesNotExist:
        return None
    return db_row
...
def hash_and_save_step(Model, app_id, hash_data_args, data_fields, message):
    ...
    fields_to_preserve = [getattr(Model, field)
                          for field in list(data_to_insert.keys())]
    ...
    try:
        ...
        print(
            f"A record with hash_id {hash_id} already exists for {Model.__name__}.")
        return None
    return record
...
def save_development_step(project, prompt_path, prompt_data, messages, llm_response):
    ...
    development_step = hash_and_save_step(
        DevelopmentSteps, project.args['app_id'], hash_data_args, data_fields, "Saved Development Step")
...
def save_user_input(project, query, user_input):
    ...
    user_input = hash_and_save_step(
        UserInputs, project.args['app_id'], hash_data_args, data_fields, "Saved User Input")
...
def get_user_input_from_hash_id(project, query):
    ...
    user_input = get_db_model_from_hash_id(
        UserInputs, project.args['app_id'], project.checkpoints['last_user_input'])
    return user_input
...
def delete_all_subsequent_steps(project):
    ...
    delete_subsequent_steps(
        CommandRuns, project.checkpoints['last_command_run'])
...
def delete_subsequent_steps(model, step):
    if step is None:
        return
    logger.info(
        colored(f"Deleting subsequent {model.__name__} steps after {step.id}", "red"))
...
def delete_unconnected_steps_from(step, previous_step_field_name):
    ...
    for unconnected_step in unconnected_steps:
        print(colored(
            f"Deleting unconnected {step.__class__.__name__} step {unconnected_step.id}", "red"))
    ...
    with database.atomic():
        for table in TABLES:
            database.execute_sql(
                f'DROP TABLE IF EXISTS "{table._meta.table_name}" CASCADE')
